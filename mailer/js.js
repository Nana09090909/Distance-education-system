...
$.ajax({type: 'POST', url:'mail.php', data:'to='+address[n-1]+'&message='+mes+'&sub='+sub+'&attach='+attach, success:function(e){if(e.indexOf('Maillist_Error:')!=-1){$('#message').html(e+'<br/><button onclick="shutdown()">Произошла ошибка отправки почты. Попробуйте перезапустить рассылку.</button>'); return;} $('#message').text(e); n=n-1; ;sendmail(address, n, attach)}});
   }
if(n==0){
...
if(n < 0){
	...
	$('#message').html('<button onclick="shutdown()">Рассылка завершена успешно</button>');
}
